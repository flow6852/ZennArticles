...
ローカルで完結するような、試したいことがでてきたら
事前に作成したテンプレートをコピーしてサービスを入れれば良いと思います。
しかし、複数のシステムを同時に構築したとき、動いてくれるか/動かないよなを試しながら
...
ホストもrouterも外部と直接通信したいが、ゲストは外部と直接通信したくないとき、
routerを経由させるために、下記nicを準備します。
...
1. ゲスト/routerが使用するための内部向けnic
    1. ゲスト用ブリッジを作成します。
    1. tunデバイスをゲスト用ブリッジに接続する形で作成します。

### router作成

routerとしたいゲストを下記コマンドにて起動し作成します。
Linuxの場合、 `net.ipv4.ip_forward = 1`を設定します。
...
ゲストマシンの仮想nicとして `kvm_mcast`を指定して作成/起動してください。
